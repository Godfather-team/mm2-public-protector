module.exports = async (req, res) => {
  ...
  // Use a named query param: ?path=<reversed_path> (client must encode special chars like / as %2F)
  const reversedPath = req.query.path;
  if (!reversedPath) {
    return res.status(400).send('Missing "path" query parameter');
  }

  // Reverse the string to get the actual webhook path
  const actualPath = reversedPath.split('').reverse().join('');

  // Construct the Discord webhook URL (or use env var for testing)
  let discordUrl = `https://discord.com/api/webhooks/${actualPath}`;
  if (process.env.TEST_WEBHOOK_URL) {
    discordUrl = process.env.TEST_WEBHOOK_URL; // Set this in your deployment env for direct testing
  }
